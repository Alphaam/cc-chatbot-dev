...
export function Presentation({ version = "original" }: { version?: DeckVersion }) {
  ...
  return <div className="presentation font-sans" ref={root}>
    <header className="deck-header"><a href="/presentation" className="deck-brand" aria-label="HR&A presentation home"><Image src="/presentation/hra-logo.svg" width={84} height={32} alt="HR&A" unoptimized /><span>Tech & Society<span className="header-subtitle">Digital product practice</span></span></a><div className="deck-tools"><nav className="edition-switch" aria-label="Presentation edition"><a href="/presentation" aria-current={version === "original" ? "page" : undefined}>Rebuilt deck <span>{decks.original.length}</span></a><a href="/presentation?draft=chatbot" aria-current={version === "chatbot" ? "page" : undefined}>Chatbot <span>{decks.chatbot.length}</span></a><a href="/presentation?draft=studio" aria-current={version === "studio" ? "page" : undefined}>Studio + tools <span>{decks.studio.length}</span></a></nav><button onClick={() => { setOverview(!overview); setNotes(false); }} aria-label={overview ? "Close slide overview" : "Open slide overview"} aria-pressed={overview} title="Slide overview">{overview ? <X /> : <Grid2X2 />}</button><button onClick={() => { setNotes(!notes); setOverview(false); }} aria-pressed={notes} aria-controls="presenter-notes" title="Presenter notes and sources" aria-label="Presenter notes and sources"><NotebookPen /></button>{version === "original" && <><a className="deck-download" href="/presentation/studio-editable.pptx" download="HRA-Tech-Society-Studio.pptx" title="Download editable PowerPoint" aria-label="Download editable PowerPoint"><ArrowDownToLine /></a><a className="deck-download" href="/presentation/studio-rebuilt.pdf" download="HRA-Tech-Society-Studio-Rebuilt.pdf" title="Download rebuilt PDF with selectable text" aria-label="Download rebuilt PDF with selectable text"><FileText /></a></>}<button onClick={() => window.print()} title="Print or save as PDF" aria-label="Print or save as PDF"><Printer /></button><button onClick={fullscreen} title="Present fullscreen" aria-label="Present fullscreen"><Maximize2 /></button></div></header>
