...
export function Device({ type = "desktop", image }: { type?: "desktop" | "tablet" | "phone"; image?: string }) {
  const label = type === "desktop" ? "Desktop monitor" : type === "tablet" ? "Tablet · portrait" : "Phone · portrait";
  return <div className={`device device-${type}`}><div className="device-hardware"><div className="device-screen"><Image src={image || `/presentation/clark-${type}.png`} alt={image ? `Actual MHM ecosystem interface in an illustrative ${label.toLowerCase()}` : `Actual Clark County assistant in an illustrative ${label.toLowerCase()}`} width={type === "desktop" ? 1440 : type === "tablet" ? 820 : 390} height={type === "desktop" ? 1000 : type === "tablet" ? 1100 : 844} unoptimized /></div>{type === "desktop" && <div className="monitor-stand" aria-hidden="true"><div /><span /></div>}</div><p className="device-label">{label}</p></div>;
}
